...
// class header{
//     private:
//         page* leftmost_ptr;         // 8 bytes 有则为inner，无则为leaf
...
// const int cardinality = (BTREE_PAGESIZE-sizeof(header))/sizeof(Record);//the number of record in a page
...
//             } while(previous_switch_counter != hdr.switch_counter);//concurrency
...
//                 if(!shift && records[i].key == key) { //除当前记录，并将其值替换为前一个记录的值或最左子节点的指针
//                     records[i].val = (i == 0) ? 
...
//                 // This node is root ，this->root != this
//                 //this 0x555556258b40
//                 //bt->root 0x555556258f00
//                 page * root = bt->root;
//                 if(this == root) {
//                     if(hdr.level > 0) {
//                         if(num_entries_before == 1 && !hdr.sibling_ptr) {//只有一个条目且没有兄弟，则上升子树
//                             // bt->root = hdr.leftmost_ptr;
//                             bt->setNewRoot(hdr.leftmost_ptr);
...
//             if(is_leftmost_node) {
//                 page * sibling = reinterpret_cast<page *>(hdr.sibling_ptr);
...
//             //兄弟关系是通过page(node)中的head管理的 ,page->the header of the page->other page
//             //page belongs to header
...
//             if(hdr.leftmost_ptr)//补偿最左子树占用空间
//                 ++total_num_entries;
...
//                     }
//                     else{
//                         //这个地方就是因为左给了右，左右的分界parent变化了，原先的下降插入右，并获得右(this)的left_most_node
...
//                         hdr.leftmost_ptr = (page*)left_sibling->records[m].val;//rise up
//                         do_flush_with_double_fence((char *)&(hdr.leftmost_ptr), sizeof(page *));

//                         left_sibling->records[m].val = NULL;//erase this record
//                         do_flush_with_double_fence((char *)&(left_sibling->records[m].val), sizeof(char *));
...
//                             ((char *)left_sibling, parent_key, (char *)this, hdr.level + 1);//insert parent_key
//                     }
//                 }
//                 else { // from leftmost case,delete this,reserve left_sibling
//                     hdr.is_deleted = 1;
...
//                                 (char*)hdr.leftmost_ptr, &left_num_entries);//左子树也一并托负过去
...
//         }
        ...
        //                 }
        //             }
        //             if(inserted==0){//new key is the minimum
...
//                     insert_key(key, (char *)right, &num_entries, flush);
//                     return this;
//                 }
//                 else {// FAIR
//                     // overflow
...
//                     _key_t split_key = records[m].key;//当前最后一个
...
//                     hdr.sibling_ptr = reinterpret_cast<page*>(sibling);
...
//                 page *t = reinterpret_cast<page*>(hdr.sibling_ptr);
...
//                                 if(off < scan_sz) {//要求数目未达到
//                                     if((tmp_ptr = current->records[0].val) != NULL) {//前后大小不变
...
//                     current = reinterpret_cast<page*>(current->hdr.sibling_ptr);
//                 }
...
//                     page* p = reinterpret_cast<page*>(t);
//                     if (p == nullptr) {
...
//             page *tt = reinterpret_cast<page*>(hdr.sibling_ptr);
...
//     page* p = root;
//     while(p->hdr.leftmost_ptr != NULL) {                              // p =root->hdr.leftmost_ptr 0x555556258b40,p->leaft == null
                                                                    
//         // p = (page *)galc->absolute(p->linear_search(key));
//         p = reinterpret_cast<page*>(p->linear_search(key));
//     }
...
//     page *t;
//     ////p = 0x555556258b40
//     //p->hdr.sibling_ptr 0x555556258df0
//     //t = 0x1c28f62d,这里的t要么是在当前p范围中有key直接返回key的payload，(_payload_t)t，要么是没找到，交给兄弟t
//     while((t = (page *)p->linear_search(key)) == p->hdr.sibling_ptr) {
//         p = t;
//         if(!p)
//             break;
...
//         return p->remove_rebalancing(this, key, false); //please check this
...
//     //p的左子树                           //当前ptr ，p比ptr高一个级别
//     if((char *)p->hdr.leftmost_ptr == reinterpret_cast<char *>(ptr)) {
...
//         if(p->records[i].val == reinterpret_cast<char *>(ptr)) {
//             if(i == 0) {
//                 if((char *)p->hdr.leftmost_ptr != p->records[i].val) {
//                     *deleted_key = p->records[i].key;
//                     // *left_sibling = (page *)galc->absolute(p->hdr.leftmost_ptr);
//                     *left_sibling = reinterpret_cast<page *>(p->hdr.leftmost_ptr);
...
//                     *left_sibling = reinterpret_cast<page *>(p->records[i - 1].val);
...
//         p = reinterpret_cast<page *>(p->linear_search(key));
//         if(p == nullptr) {
//             std::cout<<"Error: p is null"<<std::endl;
//             std::cout<<this->addr<<std::endl;
//             exit(1);
//         }
//     }
...
//     if(!t) {
//         if (ds > 0) {
//             return 2;//link is empty,cannot insert the key,we need to insert it,the learned index 
//                     //has deleted_slot，go to insert in the learned index
//         }
//         //store返回有效
...
//     first_p->update_value(key, value);//if find,renew it.
//     return 3;
...
//         p = reinterpret_cast<page*>(p->hdr.leftmost_ptr);
//     }
...
//          p = reinterpret_cast<page *>(p->hdr.sibling_ptr);
...
//                 overflow_number += sibling->hdr.last_index + 1;//本身就被用作溢出树
//             }
//             // sibling = (page *) galc->absolute(sibling->hdr.sibling_ptr);
//             sibling = reinterpret_cast<page*>(sibling->hdr.sibling_ptr);

//         }
//         // leftmost = (page *) galc->absolute(leftmost->hdr.leftmost_ptr);
//         leftmost = reinterpret_cast<page*>(leftmost->hdr.leftmost_ptr);
//     } while(leftmost);
