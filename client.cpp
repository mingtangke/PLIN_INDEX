...
//                     int logic_id;
//                     target_pos = std::stoi(command.substr(5));
//                     _key_t target_key = keys[target_pos];
//                     if (target_key >= client_table.min_key && target_key <= client_table.max_key) {
//                         logic_id = client_table.predict_pos(target_key);
//                         command = "findid " + std::to_string(target_pos) + " " + std::to_string(logic_id);
//                         find_time++;
//                     } else {
//                         command = "buff " + std::to_string(target_pos); 
//                     }
//                 }
...
//         }
//     }
//     // inline void get_table_param(int sockfd){
//         //         std::string Init_command = "cache";
//         //         if (write(sockfd, Init_command.c_str(), Init_command.length() + 1) == -1) {
//         //             std::cerr << "send error " << errno << ": " << strerror(errno) << std::endl;
//         //             exit(1);
...
//         //             // write_data_to_file(received_table,received_parameter,retrain);
//         //             received_table.clear();
//         //             received_parameter.clear();
//         //         }
//         //     }
...
void test() {
    std::vector<std::string> file_name;
    file_name.push_back("//home//ming//桌面//PLIN-N //PLIN-N//data//workload_log.csv");
    for (int k = 0; k < TEST_THREAD/40; k++) {
        std::string file_ = "command" + std::to_string(k) + ".txt";
        file_name.push_back(file_);
    }
