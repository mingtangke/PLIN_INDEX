...
//                 std::string error_message = "Invalid findid command format.\n";
//                 send(clientsocket, error_message.c_str(), error_message.length(), 0);
//                 continue;
//             }

//             int target_pos = std::stoi(command.substr(7, space_pos - 7));
//             _key_t target_key = keys[target_pos];
//             _payload_t answer;
           
//             if(prehot_cache){
//                 // // if (hot_map_.find(target_key) != hot_map_.end()) cache_hit_count++;
//                 // //for debug
//                 // std::ofstream hotkey_file("hotkey_valid.txt", std::ios_base::app);
//                 // for(auto &i:hot_map_){
//                 //     hotkey_file << std::fixed << i.first << " " << i.second << "\n";
//                 // }
//                 // exit(0);

//                 if(hot_map_.find(target_key) != hot_map_.end() &&
//                     hot_map_[target_key] == payloads[target_pos]){
//                     cache_hit_count ++;
//                     response = "Success!";
//                 }else{
//                     cache_miss_count ++;
//                     int logic_id = std::stoi(command.substr(space_pos + 1));
//                     int result = test_index.find_through_net(target_key,answer,logic_id);
//                     if(!result || answer!=payloads[target_pos]) response = "Failure!";
...
//             if(response == "Success!") test_index.db_logger.log_query(target_key,answer);
...
//         }else if(command.find("buff")!= std::string::npos){
//             int target_pos = std::stoi(command.substr(5));
//             _key_t target_key = keys[target_pos];
//             _payload_t answer;
//             std::string response;
...
//                 }else {
//                     cache_miss_count ++;
//                     int result = test_index.find_in_buffer_net(target_key,answer);
//                     if(!result|| answer!=payloads[target_pos]){
//                     response = "Failure!";
//                     }else response = "Success!";
//                 }
...
//             std::stringstream ss;
//             // std::vector<t_record>local_table = test_index.meta_table.local_table;
//             std::vector<std::vector<Param>>parameter = test_index.meta_table.parameter;
//             // serialize_vector(local_table, ss);
//             serialize_parameter(parameter, ss);
//             std::string message = ss.str();
//             send(clientsocket, message.c_str(), message.length(), 0);

//         }else if(command.find("dele")!= std::string::npos){
//             int target_pos = std::stoi(command.substr(5));
//             _key_t target_key = keys[target_pos];
//             _payload_t answer;
//             std::string response;
//             // int result = test_index.find(target_key,answer);
//             // if(result){
//             test_index.remove(target_key);
//             int result = test_index.find(target_key,answer);

//             if(!result){
//                 response = "dele success";
//             }else{
//                 response = "dele failure";
//             }
//             send(clientsocket, response.c_str(), response.length(), 0);// gym

//         }else if(command.find("upsert")!= std::string::npos){
//             size_t space_pos = command.find(' ', 7); 
//             if (space_pos == std::string::npos) {
//                 std::string error_message = "Invalid upsert command format.\n";
//                 send(clientsocket, error_message.c_str(), error_message.length(), 0);//
//                 continue;
//             }
//             _key_t upsert_key = std::stod(command.substr(7, space_pos - 7));
//             _payload_t upsert_payload = std::stoi(command.substr(space_pos + 1));

//             // upsert and check
//             _payload_t answer;
//             std::string response = "upsert succeess";
//             response = std::to_string(pthread_self()) + "Upsert key" + std::to_string(upsert_key) + " Payload: " + std::to_string(upsert_payload);
//             test_index.upsert(upsert_key, upsert_payload);
//             test_index.db_logger.log_query(upsert_key,upsert_payload);
//             // int result = test_index.find(upsert_key,answer);
//             send(clientsocket, response.c_str(), response.length(), 0);//save time//gym
//         }else{
